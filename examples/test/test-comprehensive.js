...
function runTest(testName, testFunction) {
    ...
    // Execute test with value-based error handling
    let hasError = false
    let errorObj = null
    let valueObj = null
    
    // Minimal try-catch for test execution
    try {
        valueObj = testFunction()
    } catch (e) {
        hasError = true
        errorObj = e
    }
    
    // Early return pattern for error case
    if (hasError) {
        console.log('❌ FAILED with exception:', errorObj.message, '\n')
        return
    }
    
    // Check test result value
    if (valueObj == true || valueObj == undefined) {
        console.log('✅ PASSED\n')
        testsPassed++
        return
    }
    
    console.log('❌ FAILED:', valueObj, '\n')
}
...
// Early return on module load failure
if (hasError) {
    console.error('💥 Comprehensive test failed:', errorObj)
    process.exit(1)
}

const iohook = moduleObj
